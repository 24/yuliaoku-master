...
public class ImageUtils {
    ...
    public static void openCameraImage(Activity activity) {
        Log.d("openCameraImage", "openCameraImage" + activity.toString());
        ...
        Log.d("openCameraImage", "startIntent" + activity.toString());
...
//        Log.i("", "生成的照片输出路径：" + imageFilePath.toString());
        File folder = new File(StaticConstant.cachePath);
        if (!folder.exists())
            folder.mkdirs();
        File file = new File(StaticConstant.cachePath + "/e5785589262");
        if (file.exists())
            file.delete();
